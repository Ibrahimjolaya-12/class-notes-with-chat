...
// 3. Access / Create Direct Chat (Unhide for both participants if hidden)
export const accessDirectChat = async (req, res) => {
  try {
    const { recipientId } = req.body;
    const myId = req.user?._id || req.user?.id;
    ...
    if (!chatRoom) {
      ...
      // Agar kisi ne bhi hide ki thi, toh dobara chat open karne par unhide kar do
      if (chatRoom.hiddenFor && chatRoom.hiddenFor.length > 0) {
        chatRoom.hiddenFor = [];
        await chatRoom.save();
      }
    }
...
export const uploadChatFile = async (req, res) => {
  try {
    ...
    const cloudinaryResponse = await uploadOnCloudinary(req.file.path);
    if (!cloudinaryResponse?.secure_url) {
      return res.status(500).json({ success: false, message: "Upload file upload failed" });
    }
